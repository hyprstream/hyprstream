...
use hyprstream_core::{
    aggregation::{AggregateFunction, AggregationView, BatchAggregation, GroupBy, TimeWindow},
    storage::{DuckDbBackend, StorageBackend},
};
...
async fn test_backend_insert_and_query() {
    ...
    // Query data
    let result = backend
        .query_sql("SELECT * FROM test_metrics")
        .await
        .unwrap();
...
async fn test_backend_aggregation() {
    ...
    // Query aggregation
    let result = backend
        .query_sql("SELECT * FROM test_agg_view")
        .await
        .unwrap();
    assert_eq!(result.num_rows(), 1);
}
