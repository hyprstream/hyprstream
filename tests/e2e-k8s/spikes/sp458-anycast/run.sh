...
log "cluster=${CLUSTER_NAME} namespace=${NS} flow=${FLOW} replicas=${REPLICAS} flap=${FLAP} relay=${RELAY_IMAGE}"
...
relay_image_escaped="${RELAY_IMAGE//&/\\&}"
sed "s/__REPLICAS__/${REPLICAS}/; s|__IMAGE__|${relay_image_escaped}|" "${SCRIPT_DIR}/relay.yaml" \
  | kubectl --kubeconfig "$KUBECONFIG" -n "$NS" apply -f -
kubectl --kubeconfig "$KUBECONFIG" -n "$NS" apply -f "${SCRIPT_DIR}/service.yaml"
kubectl --kubeconfig "$KUBECONFIG" -n "$NS" wait --for=condition=Available deployment/relay --timeout=180s
VIP="$(kubectl --kubeconfig "$KUBECONFIG" -n "$NS" get svc relay -o jsonpath='{.spec.clusterIP}')"
log "anycast VIP = ${VIP}"
...
kubectl --kubeconfig "$KUBECONFIG" -n "$NS" run "${CLIENT_POD}" --restart=Never \
  --image="${RELAY_IMAGE}" --command -- \
  sh -c 'echo "client-ready"; sleep 3600'
kubectl --kubeconfig "$KUBECONFIG" -n "$NS" wait --for=condition=Ready "pod/${CLIENT_POD}" --timeout=120s
...
demonstrate_flap_mechanism() {
  ...
  local before after
  before="$(kubectl --kubeconfig "$KUBECONFIG" -n "$NS" get endpoints relay \
            -o jsonpath='{.subsets[0].addresses[*].targetRef.name}' 2>/dev/null | tr ' ' ',')"
  local victim
  victim="$(kubectl --kubeconfig "$KUBECONFIG" -n "$NS" get pods -l app=relay -o jsonpath='{.items[0].metadata.name}')"
  log "  endpoints before: ${before:-<none>}"
  log "  evicting ${victim} (force Service reroute)..."
  kubectl --kubeconfig "$KUBECONFIG" -n "$NS" delete pod "${victim}" --grace-period=0 --force >/dev/null
  # Endpoint controller + ReplicaSet reconcile: wait for a replacement, then re-read.
  kubectl --kubeconfig "$KUBECONFIG" -n "$NS" wait --for=condition=Available deployment/relay --timeout=120s >/dev/null
  after="$(kubectl --kubeconfig "$KUBECONFIG" -n "$NS" get endpoints relay \
           -o jsonpath='{.subsets[0].addresses[*].targetRef.name}' 2>/dev/null | tr ' ' ',')"
...
if ! kubectl --kubeconfig "$KUBECONFIG" -n "$NS" exec "$CLIENT_POD" -- sh -c 'command -v quic-flow-probe' >/dev/null 2>&1; then
...
BACKEND_BEFORE="$(kubectl --kubeconfig "$KUBECONFIG" -n "$NS" exec "$CLIENT_POD" -- \
  quic-flow-probe establish --addr "${VIP}" --flow "${FLOW}")"
log "flow established, serving backend = ${BACKEND_BEFORE}"

# 3. FORCE the flap: evict the serving pod so the Service reroutes.
SERVING_POD="$(kubectl --kubeconfig "$KUBECONFIG" -n "$NS" get pod "${BACKEND_BEFORE}" \
  -o jsonpath='{.metadata.name}' 2>/dev/null || true)"
[[ -n "$SERVING_POD" ]] || die "serving backend '${BACKEND_BEFORE}' did not match a relay pod"
log "flap (${FLAP}): forcing Service away from ${SERVING_POD:-serving pod}"
case "$FLAP" in
  delete)
    kubectl --kubeconfig "$KUBECONFIG" -n "$NS" delete pod "${SERVING_POD}" --grace-period=0 --force >/dev/null
    ;;
  endpoints)
    # Rewrite the Endpoints to drop the serving pod — pure control-plane flap.
    kubectl --kubeconfig "$KUBECONFIG" -n "$NS" patch endpoints relay --type=json \
      -p='[{"op":"remove","path":"/subsets/0/addresses/0"}]' >/dev/null
    ;;
esac

ready_relay_endpoints() {
  kubectl --kubeconfig "$KUBECONFIG" -n "$NS" get endpointslices \
    -l kubernetes.io/service-name=relay -o json \
    | python3 -c '
import json
import sys
...
wait_backend_transition() {
  local victim="$1"
  local endpoints
  for _ in $(seq 1 60); do
    endpoints="$(ready_relay_endpoints)" || return 1
    if [[ -n "$endpoints" ]] && ! grep -qx "$victim" <<<"$endpoints"; then
      return 0
    fi
    sleep 1
  done
...
wait_backend_transition "$SERVING_POD" || die "serving backend ${SERVING_POD} still present after flap timeout"

# 4. OBSERVE the live flow across the flap.
log "observing flow across flap..."
if kubectl --kubeconfig "$KUBECONFIG" -n "$NS" exec "$CLIENT_POD" -- quic-flow-probe alive --flow "${FLOW}"; then
  log "RESULT: flow SURVIVED the backend flap (QUIC connection migration / moq recovery held)"
  BACKEND_AFTER="$(kubectl --kubeconfig "$KUBECONFIG" -n "$NS" exec "$CLIENT_POD" -- quic-flow-probe backend --flow "${FLOW}")"
