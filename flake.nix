...
#   nix develop                    # dev shell
{
  ...
  outputs = { self, nixpkgs, flake-utils, crane, fenix }:
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in {
        # GPU variants depend on cudaPackages/rocmPackages, which only exist on
        # x86_64-linux. Guard them so evaluation (nix flake show/check) succeeds on
        # other systems (macOS, aarch64) with just the CPU variant present.
        packages = {
        ...
        in pkgs.mkShell {
          ...
          shellHook = ''
            # inputsFrom pulls the package's build env into the shell, including the
            # sccache wrapper config. Drop it so `cargo build` in `nix develop` works
            # without sccache and the /var/cache/sccache sandbox path being present.
