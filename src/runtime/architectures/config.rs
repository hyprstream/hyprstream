...
mod tests {
    ...
    fn test_attention_type_detection() {
        ...
        impl ArchitectureConfig for TestConfig {
            fn num_attention_heads(&self) -> usize { self.heads }
            fn num_key_value_heads(&self) -> usize { self.kv_heads }
            fn hidden_size(&self) -> usize { 4096 }
            fn intermediate_size(&self) -> usize { 11008 }
            fn vocab_size(&self) -> usize { 32000 }
            fn max_position_embeddings(&self) -> usize { 4096 }
            fn rope_theta(&self) -> Option<f32> { Some(10000.0) }
            fn rope_dim(&self) -> Option<usize> { None }
            fn layer_norm_eps(&self) -> f32 { 1e-6 }
            fn use_rms_norm(&self) -> bool { true }
        }
        
        // MHA: equal heads
        let config = TestConfig { heads: 32, kv_heads: 32 };
        assert_eq!(config.attention_type(), AttentionType::MHA);
        
        // MQA: single KV head
        let config = TestConfig { heads: 32, kv_heads: 1 };
        assert_eq!(config.attention_type(), AttentionType::MQA);
        
        // GQA: grouped heads
        let config = TestConfig { heads: 32, kv_heads: 8 };
        assert_eq!(config.attention_type(), AttentionType::GQA { num_groups: 4 });
    }
    ...
    fn test_kv_expansion_factor() {
        ...
        assert_eq!(AttentionType::GQA { num_groups: 4 }.kv_expansion_factor(32), 8);
    }
}