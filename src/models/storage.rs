    ...
    async fn init_tables(&self) -> Result<(), Status> {
        ...
        match &*self.backend {
            HyprStorageBackendType::Adbc(backend) => {
                backend.create_table("model_metadata", &get_model_metadata_schema()).await?;
                backend.create_table("model_layers", &get_model_layer_schema()).await?;
            }
...
impl TimeSeriesModelStorage {
    ...
    fn record_batch_to_layers(batch: RecordBatch) -> Result<Vec<ModelLayer>, Status> {
        ...
        log::info!("Converting record batch to ModelLayer instances");
