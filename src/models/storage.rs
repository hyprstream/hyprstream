...
impl TimeSeriesModelStorage {
    ...
    async fn init_tables(&self) -> Result<(), Status> {
        ...
        self.backend
            .as_ref()
            .as_ref()
            .create_table("model_metadata", &get_model_metadata_schema())
            .await?;
        self.backend
            .as_ref()
            .as_ref()
            .create_table("model_layers", &get_model_layer_schema())
            .await?;
        Ok(())
    }
...
impl ModelStorage for TimeSeriesModelStorage {
    ...
    async fn store_model(&self, model: &Model) -> Result<(), Status> {
        ...
        for batch in batches {
            self.backend
                .insert_into_table("model_layers", batch)
                .await?;
        }
...
impl TimeSeriesModelStorage {
    fn record_batches_to_model(
        metadata_batch: RecordBatch,
        layers_batch: RecordBatch,
    ) -> Result<Model, Status> {
    ...
    fn record_batch_to_layers(batch: RecordBatch) -> Result<Vec<ModelLayer>, Status> {
        ...
        for row_idx in 0..batch.num_rows() {
            ...
            let parameters: HashMap<String, String> = bincode::deserialize(params.value(row_idx))
                .map_err(|e| {
                Status::internal(format!("Failed to deserialize parameters: {}", e))
            })?;
    ...
    fn record_batch_to_metadata_row(
        batch: &RecordBatch,
        row_idx: usize,
