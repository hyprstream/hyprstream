...
impl DuckDbBackend {
    ...
    fn prepare_params(metrics: &[HyprMetricRecord]) -> Result<RecordBatch, Status> {
        ...
        let counts = Int64Array::from_iter_values(metrics.iter().map(|m| m.value_running_window_count as i64));
...
impl DuckDbBackend {
    ...
    fn arrow_type_to_duckdb_type(data_type: &DataType) -> Result<&'static str, Status> {
        Ok(match data_type {
            ...
            _ => return Err(Status::invalid_argument(format!("Unsupported Arrow data type: {:?}", data_type)))
        })
    }

    /// Converts an Arrow schema to a DuckDB CREATE TABLE statement
    fn schema_to_create_table_sql(table_name: &str, schema: &Schema) -> Result<String, Status> {
        ...
        for field in schema.fields() {
            ...
            let duckdb_type = Self::arrow_type_to_duckdb_type(field.data_type())?;
            sql.push_str(&format!("\"{}\" {}", field.name(), duckdb_type));
        }
    ...
    fn convert_to_arrow_array(values: &[duckdb::types::Value], data_type: &DataType) -> Result<ArrayRef, Status> {
        match data_type {
            ...
            _ => Err(Status::invalid_argument(format!("Unsupported data type: {:?}", data_type))),
        }
    }
