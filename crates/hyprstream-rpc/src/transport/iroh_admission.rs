...
//! `did:at9p` peer, the GATE-verified capsule keys (D2 / #894, once wired). It
//! **never** reads a pkarr record: pkarr is a liveness/reach hint on the
//! mainline DHT and derives zero authority (see
//! [`crate::transport::iroh_substrate`]'s "D3" note). The reach a pkarr record
//! advertises may be what lets us *dial* a peer, but it plays no part in the
//! *admit/deny* decision — that is the liveness-only invariant D3 pins.
