...
//! wasmtime substrate ([`hyprstream_wasm::Sandbox`]). There is no hypervisor and
...
impl SandboxBackend for WasmBackend {
    ...
    async fn exec_sync(
        ...
        // Map the command to a guest invocation. Two shapes:
        //   ["eval", "<python/source>"]  → Sandbox::eval(source, fuel)
        //   ["eval", "<source>", "<ticks>"] → epoch-bounded eval
        // `eval` is the substrate's canonical pyguest export (it ships a source
        // string into guest memory and calls the `eval` export). The exit code is
        // the guest's returned status (0 = ok, nonzero = guest-level error). A
        // wasm trap (fuel/epoch/dead-import) surfaces as exit code 124-style error.
        if command.is_empty() {
            return Err(WorkerError::ExecFailed(
                "exec_sync: empty command (expected [\"eval\", <source>, ..])".into(),
            ));
        }

        let export = command[0].as_str();
        match export {
            "eval" => {
                let source = command.get(1).cloned().unwrap_or_default();
                // Run the (CPU-bound, blocking) wasm call off the async reactor.
                // The substrate Sandbox is Send+Sync; clone the Arc into the task.
                let join = tokio::task::spawn_blocking(move || sb.eval(&source, fuel));
                ...
                match result {
                    ...
                    // A trap (out of fuel, epoch interrupt, or a dead import being
                    // called) is a sandbox-level failure: nonzero exit + stderr.
                    Err(trap) => {
            ...
            other => Err(WorkerError::ExecFailed(format!(
                "wasm exec_sync: unsupported export '{other}'. The P2 substrate \
                 exposes the canonical 'eval' guest entrypoint; map your command \
                 to [\"eval\", <source>]. Generic typed-export invocation is \
                 TODO(#505)."
            ))),
        }
    }
...
// (which is what pulls in the wasmtime-bearing `hyprstream-wasm` dep). Mirrors
...
mod tests {
    ...
    #[tokio::test]
    async fn exec_sync_unsupported_export_errors() {
        let backend = WasmBackend::new(WasmConfig::default());
        let b64 = trivial_guest_b64();
        let cfg = pod_config("tenant-1", vec![(ANN_WASM_BASE64, &b64)]);
        ...
        let err = backend
            ...
            .await
            .unwrap_err();
        assert!(err.to_string().contains("unsupported export"), "got: {err}");
    }
