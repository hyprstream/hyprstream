//! Postgres-backed KV store for the PDS record store and the discovery
//! accepted-state authority (#1257).
//!
//! This module implements the RDS Multi-AZ backend as a projection-free
//! BYTEA-keyed KV shell. It is selected when the `[rds]` config section (or
//! its role-scoped env binding — see [`crate::rds::RdsConfig`]) is set and the
//! `postgres` cargo feature is enabled. When unset, callers keep their local
//! RocksDB backend.
//!
//! It lives in `hyprstream-pds` so both the application record store
//! (`PdsRecordStore`) and the discovery crate's checkpointed accepted-state
//! authority share ONE audited connection/TLS/pool implementation. No
//! divergent copy may exist.
//!
...
pub struct SnapshotRead {
    pub ranges: Vec<Vec<(Vec<u8>, Vec<u8>)>>,
...
impl PgKv {
    ...
    /// startup path. Mirrors the RocksDB reader's
    /// `open_for_read_only(create_if_missing = false)`: an unprovisioned
    /// store (no `pds_kv` table) is indistinguishable from lost security
    /// history, so it fails closed instead of recreating anything.
    ...
    /// `connect_test` counterpart). Production read-only connections MUST go
    /// through [`PgKv::connect_readonly`], which enforces the verify-full
    /// contract.
    #[cfg(any(test, feature = "postgres-test-connect"))]
    pub fn connect_test_readonly(driver_url: &str) -> AnyResult<Self> {
    ...
    ///
    /// Also compiled under the `postgres-test-connect` feature so downstream
    /// crates' integration tests can reach a scratch database; no production
    /// feature enables it.
    #[cfg(any(test, feature = "postgres-test-connect"))]
    pub fn connect_test(driver_url: &str, cell_id: &str) -> AnyResult<Self> {
        let mut pg_config: tokio_postgres::Config = driver_url
            .parse()
            .map_err(|e| anyhow::anyhow!("test Postgres URL did not parse: {e}"))?;
        apply_driver_timeouts(&mut pg_config);
    ...
    pub fn get(&self, key: &[u8]) -> AnyResult<Option<Vec<u8>>> {
    ...
    pub fn get_batch(&self, keys: &[Vec<u8>]) -> AnyResult<Vec<Option<Vec<u8>>>> {
    ...
    pub fn put(&self, key: &[u8], value: &[u8]) -> AnyResult<()> {
    ...
    pub fn all_pairs(&self) -> AnyResult<Vec<(Vec<u8>, Vec<u8>)>> {
    ...
    rt.block_on(async move {
        ...
        if let Err(e) = provision {
            ...
            while let Some(cmd) = cmd_rx.recv().await {
                let _ = send_err(cmd, anyhow::anyhow!("provisioning check failed: {e}"));
            }
            return;
        }
...
/// Read-only provisioning gate: the `pds_kv` table must already exist. The
/// resolver startup path never creates security state — a missing table means
/// the deployment was never provisioned (or lost its history), which is
/// fatal, not creatable.
async fn verify_provisioned(pool: &deadpool_postgres::Pool) -> AnyResult<()> {
    let conn = pool
        .get()
        .await
        .map_err(|e| anyhow::anyhow!("RDS provisioning check: connection acquisition failed: {e}"))?;
    let row = conn
        .query_opt("SELECT to_regclass('pds_kv') IS NOT NULL", &[])
        .await
        .map_err(|e| anyhow::anyhow!("RDS provisioning check: query failed: {e}"))?;
    let provisioned = row
        .map(|r| r.get::<_, bool>(0))
        .ok_or_else(|| anyhow::anyhow!("RDS provisioning check: empty result"))?;
    anyhow::ensure!(
        provisioned,
        "checkpointed PDS Postgres store is not provisioned (no pds_kv table);          refusing to recreate accepted-state history at resolver startup"
    );
    Ok(())
}
...
mod tests {
    ...
    fn translated_url(url: &str) -> crate::rds::ValidatedRdsUrl {
        crate::rds::RdsConfig::validate_url(url)
