...
//!
//! Build the guest first (else these tests SKIP):
...
use hyprstream_wasm::wasi_sandbox::WasiSandbox;
...
fn guest_wasm() -> Option<Vec<u8>> {
    ...
    let guest_dir = manifest.parent().map(|p| p.join("hyprstream-wasm-fsguest"))?;
...
fn wasi_guest_filesystem_is_the_vfs_mount() {
    let Some(wasm) = guest_wasm() else {
        eprintln!("SKIP profile_b: fsguest wasm not built (cargo build --release --target wasm32-wasip1 --manifest-path crates/hyprstream-wasm-fsguest/Cargo.toml)");
        return;
    };
    ...
    let out = files.get("out.txt").expect("out.txt must exist in the Mount");
    assert_eq!(out, b"sfv-olleh", "out.txt = reversed seed, round-tripped via VFS");
    let done = files.get("done.txt").expect("done.txt must exist in the Mount");
    assert_eq!(done, b"ok", "done.txt success marker landed in the VFS Mount");
...
fn wasi_guest_subject_reaches_mount_policy_point() {
    let Some(wasm) = guest_wasm() else {
        eprintln!("SKIP profile_b: fsguest wasm not built");
        return;
    };
    let mount = Arc::new(MemMount::new(vec![("seed.txt", b"hello-vfs")]));
    ...
    let handle = std::thread::spawn(move || {
        let sandbox = WasiSandbox::wasi_for(&wasm, denied, mount as Arc<dyn Mount>)
