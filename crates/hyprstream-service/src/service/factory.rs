...
impl ServiceContext {
    ...
    /// Publish a bound native Iroh carrier for a non-RPC service such as the
    /// Event plane.  This uses the same checkpoint-verified authority bundle
    /// and cancellation ownership as `into_spawnable_quic`; it is intentionally
    /// unavailable without a complete native announcement.
    pub fn native_iroh_announcement_callback(
        &self,
        service_name: &str,
    ) -> anyhow::Result<Arc<dyn Fn(tokio_util::sync::CancellationToken, [u8; 32]) -> anyhow::Result<()> + Send + Sync>> {
        let shared = self
            .quic_shared
            .as_ref()
            .ok_or_else(|| anyhow::anyhow!("native Iroh publication requires QUIC configuration"))?;
        ...
        let policy_verifying_key = trust
            .resolve_one("policy")
            .ok_or_else(|| anyhow::anyhow!("trust store has no policy key"))?;
        let discovery_verifying_key = trust
            .resolve_one("discovery")
            .ok_or_else(|| anyhow::anyhow!("trust store has no discovery key"))?;
        anyhow::ensure!(
            trust.is_authorized(&discovery_verifying_key, "discovery"),
            "trust store has no authorized discovery key"
        );
        ...
        Ok(Arc::new(move |cancellation, node_id| {
            publish_native_announcement(
                publisher.clone(),
                cancellation,
                service_name.clone(),
