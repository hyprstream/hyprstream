...
impl PlacementIndex {
    ...
    pub async fn ingest_did(&self, resolver: &dyn RecordResolver, did: &str) -> Result<()> {
        ...
        // `Ok(None)` keeps the trusted-resolver posture (no key resolution).
    ...
    /// is `Some`, the CAR's commit signature is additionally verified against it
    /// via `Commit::verify` (the verified-by-construction gate of #932): the
