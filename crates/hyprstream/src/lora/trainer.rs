...
impl LoRATrainer {
    ...
    pub fn new(vs: &nn::VarStore, device: Device, config: TrainingConfig) -> Result<Self> {
        // Create AdamW optimizer for LoRA parameters
        let optimizer = nn::AdamW::default().build(vs, config.learning_rate)?;
    ...
    fn update_learning_rate(&mut self) {
        if self.global_step < self.config.warmup_steps {
            ...
            let progress =
                (self.global_step - self.config.warmup_steps) as f64 / estimated_total_steps as f64;
            let cosine_lr = self.config.learning_rate
                * (1.0 + progress.min(1.0) * std::f64::consts::PI).cos()
                / 2.0;
    ...
    pub fn load_checkpoint(&mut self, lora_model: &mut LoRAModel, path: &str) -> Result<()> {
        ...
        tracing::info!(
            "Loaded checkpoint from {} at step {}",
            path,
            self.global_step
        );
        Ok(())
