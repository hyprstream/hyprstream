...
pub enum SyntheticNode {
    ...
    /// Status file backed by `tokio::sync::watch`. Always returns the latest value.
    /// Stores the Sender so receivers can be cloned per-fid at walk time.
    /// Uses `bytes::Bytes` for O(1) reference-counted clone on the read path.
    WatchFile(tokio::sync::watch::Sender<bytes::Bytes>),
    /// Completion file backed by `tokio::sync::OnceCell`. First reader resolves;
    /// all subsequent readers get the cached value.
    OneshotFile(Arc<OneshotState>),
    /// Async ctl file with an async handler. Used for operations that need `.await`
    /// (e.g., forwarding to remote services). Must resolve as `ResolvedNode::Static`
    /// to avoid holding DashMap guards across await points.
    AsyncCtlFile {
        handler: Box<dyn Fn(Vec<u8>, hyprstream_rpc::Subject) -> Pin<Box<dyn Future<Output = Result<Vec<u8>, String>> + Send>> + Send + Sync>,
    },
}

/// State for a `OneshotFile` node.
///
/// Cancellation-safe: only successful results are cached in the `OnceCell`.
/// If the sender is dropped before sending, `read()` returns an error but does
/// not poison the file — a new sender could be wired up (though in practice
/// oneshot channels are single-use).
pub struct OneshotState {
    cell: tokio::sync::OnceCell<bytes::Bytes>,
    rx: tokio::sync::Mutex<Option<tokio::sync::oneshot::Receiver<bytes::Bytes>>>,
}

impl OneshotState {
    /// Create a new OneshotState. The returned oneshot Sender resolves the file.
    pub fn new() -> (Arc<Self>, tokio::sync::oneshot::Sender<bytes::Bytes>) {
    ...
    /// Read the value. First caller takes the receiver and awaits it;
    /// all subsequent callers get the cached result.
    ///
    /// Only `Ok` values are cached. Errors (sender dropped) are transient —
    /// the `OnceCell` remains uninitialised so a future call could still succeed
    /// if the receiver is somehow replaced (not typical for oneshot, but
    /// prevents permanent poisoning).
    pub async fn read(&self) -> Result<bytes::Bytes, String> {
        // Fast path: already resolved.
        if let Some(data) = self.cell.get() {
            return Ok(data.clone());
        }

        // Slow path: take the receiver (brief lock, not held across await).
        let rx = {
        ...
        match rx {
            Some(rx) => match rx.await {
                Ok(data) => {
                    // Cache the successful result. If another task raced us,
                    // set() returns Err but the value is already cached — fine.
                    let _ = self.cell.set(data.clone());
                    Ok(data)
                }
                Err(_) => Err("oneshot sender dropped".to_owned()),
            },
            None => {
                // Receiver already taken by another caller — check if they cached a result.
                match self.cell.get() {
                    Some(data) => Ok(data.clone()),
                    None => Err("oneshot pending — another reader is resolving".to_owned()),
                }
            }
        }
    }
    ...
    fn read_from_node(&self, node: Option<&SyntheticNode>, offset: u64, count: u32) -> Result<Vec<u8>, String> {
        match node {
            ...
            Some(SyntheticNode::WatchFile(sender)) => {
                ...
                // because `sender.borrow()` is non-blocking (returns a `Ref` immediately)
                // and `Bytes::clone()` is O(1) (reference count bump). The Ref is dropped
                // before returning. No contention risk.
                let data = sender.borrow().clone();
...
impl hyprstream_vfs::Mount for SyntheticTree {
    ...
    async fn read(&self, fid: &hyprstream_vfs::Fid, offset: u64, count: u32, caller: &hyprstream_rpc::Subject) -> Result<Vec<u8>, hyprstream_vfs::MountError> {
        ...
        // Check for OneshotFile — requires async read. Extract the Arc<OneshotState>
        // from the DashMap guard before awaiting to avoid holding !Send guard across .await.
        {
            let entry = self.fid_table.get_verified(fid_u32, &owner)
                .map_err(|e| hyprstream_vfs::MountError::Io(e.to_owned()))?;
            if let FidState::Opened { resolved, .. } = &entry.state {
                let node = match resolved {
                    ResolvedNode::Static(path) => self.resolve_node(path),
                    ResolvedNode::Dynamic(node) => Some(node),
                };
                if let Some(SyntheticNode::OneshotFile(state)) = node {
    ...
    async fn write(&self, fid: &hyprstream_vfs::Fid, offset: u64, data: &[u8], caller: &hyprstream_rpc::Subject) -> Result<u32, hyprstream_vfs::MountError> {
        ...
        // Check for AsyncCtlFile — requires async handler. Extract handler, drop guard, await.
        {
            let entry = self.fid_table.get_verified(fid_u32, &owner)
                .map_err(|e| hyprstream_vfs::MountError::Io(e.to_owned()))?;
            if let FidState::Opened { resolved, mode, .. } = &entry.state {
                let base_mode = *mode & 0x03;
                if base_mode != hyprstream_vfs::OWRITE && base_mode != hyprstream_vfs::ORDWR {
                    return Err(hyprstream_vfs::MountError::Io("fid not open for writing".to_owned()));
                }
                let node = match resolved {
                    ResolvedNode::Static(path) => self.resolve_node(path),
                    ResolvedNode::Dynamic(node) => Some(node),
                };
                if let Some(SyntheticNode::AsyncCtlFile { handler }) = node {
                    let fut = handler(data.to_vec(), caller.clone());
                    let written = data.len() as u32;
                    ...
                    if let Ok(entry) = self.fid_table.get_verified(fid_u32, &owner) {
                        if let FidState::Opened { response_buf, .. } = &entry.state {
                            let mut buf = response_buf.lock();
                            buf.clear();
                            let cap = Self::MAX_RESPONSE_BUF.min(result.len());
...
mod tests {
    ...
    // ── Phase 3: Channel-backed variant tests ─────────────────────────────

    #[tokio::test]
    async fn watch_file_returns_latest_value() {
        use hyprstream_vfs::{Namespace, Subject};

        let (tx, _rx) = tokio::sync::watch::channel(bytes::Bytes::from_static(b"initial"));
        let mut children = HashMap::new();
        children.insert("status".to_owned(), SyntheticNode::WatchFile(tx.clone()));
        let tree = SyntheticTree::new(SyntheticNode::Dir { children });

        let mut ns = Namespace::new();
        ns.mount("/srv", Arc::new(tree)).unwrap();
        let caller = Subject::new("alice");

        // Read initial value.
        let data = ns.cat("/srv/status", &caller).await.unwrap();
        assert_eq!(data, b"initial");

        // Update via sender.
        tx.send(bytes::Bytes::from_static(b"updated")).unwrap();
    ...
    #[tokio::test]
    async fn oneshot_file_resolves_once() {
        use hyprstream_vfs::{Namespace, Subject};

        let (state, sender) = OneshotState::new();
        let mut children = HashMap::new();
        children.insert("result".to_owned(), SyntheticNode::OneshotFile(state));
        let tree = SyntheticTree::new(SyntheticNode::Dir { children });
        ...
        // Send the value.
        sender.send(bytes::Bytes::from_static(b"done")).unwrap();

        // First read resolves the oneshot.
        let data = ns.cat("/srv/result", &caller).await.unwrap();
    ...
    #[tokio::test]
    async fn async_ctl_file_round_trip() {
        use hyprstream_vfs::{Namespace, Subject};

        let mut children = HashMap::new();
        children.insert("ctl".to_owned(), SyntheticNode::AsyncCtlFile {
            handler: Box::new(|data, _subject| {
                Box::pin(async move {
                    let cmd = String::from_utf8_lossy(&data);
                    Ok(format!("async: {}", cmd.trim()).into_bytes())
                })
            }),
        });
        let tree = SyntheticTree::new(SyntheticNode::Dir { children });
        ...
        let resp = ns.ctl("/srv/ctl", b"ping", &caller).await.unwrap();
        assert_eq!(resp, b"async: ping");
    }
}
