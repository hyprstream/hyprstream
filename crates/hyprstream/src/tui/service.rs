    ...
    async fn handle_spawn_chrome_shell(
        ...
        let models = {
            let registry_client: crate::services::RegistryClient =
                crate::services::RegistryClient::from_installed_resolver(
                    self.signing_key.clone(),
                    registry_vk,
                    None,
                )?;
            let model_client_for_status = crate::services::generated::model_client::ModelClient::from_installed_resolver(
                self.signing_key.clone(),
                model_vk,
                None,
            )?;
        ...
        let load_fn: Box<dyn Fn(&str, hyprstream_tui::shell_app::ModelStatusSender) + Send> =
            Box::new(move |model_ref: &str, tx: hyprstream_tui::shell_app::ModelStatusSender| {
                ...
                h.block_on(async {
                    let client = match crate::services::generated::model_client::ModelClient::from_installed_resolver(
                        sk.clone(),
                        vk,
                        None,
                    ) {
        ...
        let git_ops = {
            ...
            let clone_fn: hyprstream_tui::shell_app::CloneFn = Box::new(move |url, name, branch, tx: GitProgressSender| {
                ...
                std::thread::spawn(move || {
                    h.block_on(async {
                        let registry = match crate::services::RegistryClient::from_installed_resolver(sk, rvk, None) {
                            Ok(c) => c,
                            Err(e) => {
            ...
            let refresh_fn: hyprstream_tui::shell_app::RefreshFn = Box::new(move |tx: GitProgressSender| {
                ...
                std::thread::spawn(move || {
                    h.block_on(async {
                        let registry_client = match crate::services::RegistryClient::from_installed_resolver(sk.clone(), rvk, None) {
                            Ok(c) => c,
                            Err(e) => {
                        ...
                        let model_client = match crate::services::generated::model_client::ModelClient::from_installed_resolver(sk, mvk, None) {
                            Ok(c) => c,
                            Err(e) => {
