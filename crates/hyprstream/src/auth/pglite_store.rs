...
CREATE TABLE IF NOT EXISTS userstore_schema_migrations (
    version BIGINT PRIMARY KEY,
    state TEXT NOT NULL CHECK (state IN ('in_progress', 'complete')),
    started_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    completed_at TIMESTAMPTZ
);
CREATE TABLE IF NOT EXISTS userstore_migration_progress (
    version BIGINT NOT NULL
        REFERENCES userstore_schema_migrations(version) ON DELETE CASCADE,
    username TEXT NOT NULL REFERENCES users(username) ON DELETE CASCADE,
...
const ENCRYPTION_MIGRATION_VERSION: i64 = 1;
static ENCRYPTION_MIGRATION_LOCK: tokio::sync::Mutex<()> = tokio::sync::Mutex::const_new(());

/// During a resumable migration, both the exact #1385 plaintext sizes and the
/// exact encrypted sizes are accepted. The algorithm/PQ pairing invariant is
/// preserved throughout; it is never dropped without its replacement being
/// installed in the same transaction.
const BEGIN_ENCRYPTION_MIGRATION_SQL: &str = r#"
INSERT INTO userstore_schema_migrations(version, state)
...
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_name_storage_check;
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_email_storage_check;
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_external_id_storage_check;
ALTER TABLE pubkeys DROP CONSTRAINT IF EXISTS pubkeys_pubkey_storage_check;
ALTER TABLE pubkeys DROP CONSTRAINT IF EXISTS pubkeys_label_storage_check;
ALTER TABLE pubkeys DROP CONSTRAINT IF EXISTS pubkeys_algorithm_pq_storage_check;
ALTER TABLE pubkeys ADD CONSTRAINT pubkeys_pubkey_storage_check CHECK (
...
ALTER TABLE pubkeys ADD CONSTRAINT pubkeys_algorithm_pq_storage_check CHECK (
    (algorithm = 'ed25519' AND pq_pubkey IS NULL)
    OR (
        algorithm = 'ed25519+ml-dsa-65'
        AND pq_pubkey IS NOT NULL
        AND (
            octet_length(pq_pubkey) = 1952
            OR (
                octet_length(pq_pubkey) = 1984
                AND substring(pq_pubkey from 1 for 4) = decode('48534331', 'hex')
            )
        )
    )
...
const FINALIZE_ENCRYPTION_MIGRATION_SQL: &str = r#"
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_name_storage_check;
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_email_storage_check;
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_external_id_storage_check;
ALTER TABLE users ADD CONSTRAINT users_name_storage_check CHECK (
    name IS NULL OR (
        octet_length(name) >= 32
        AND substring(name from 1 for 4) = decode('48534331', 'hex')
    )
);
ALTER TABLE users ADD CONSTRAINT users_email_storage_check CHECK (
    email IS NULL OR (
        octet_length(email) >= 32
        AND substring(email from 1 for 4) = decode('48534331', 'hex')
    )
);
ALTER TABLE users ADD CONSTRAINT users_external_id_storage_check CHECK (
    external_id IS NULL OR (
        octet_length(external_id) >= 32
        AND substring(external_id from 1 for 4) = decode('48534331', 'hex')
    )
...
ALTER TABLE pubkeys ADD CONSTRAINT pubkeys_label_storage_check CHECK (
    label IS NULL OR (
        octet_length(label) >= 32
        AND substring(label from 1 for 4) = decode('48534331', 'hex')
    )
...
UPDATE userstore_schema_migrations
...
impl PgliteUserStore {
    ...
    ///
    /// Deployment key material is loaded before the database opens, and the
    /// store is not returned until the resumable encryption migration is
    /// complete. There is no production constructor that can omit encryption.
    ...
    async fn with_cipher(database: Arc<PGlite>, cipher: ColumnCipher) -> Result<Self> {
        ...
        let store = Self { database, cipher };
        store
            .migrate_encrypted_columns()
            .await
            .context("migrating UserStore value columns to encrypted storage")?;
        Ok(store)
    }
    ...
    // ── Versioned plaintext-to-ciphertext migration ──────────────────────

    async fn migrate_encrypted_columns(&self) -> Result<()> {
        // PGlite is a process singleton, but multiple repositories may be
        // constructed over its shared Arc. Serialize the multi-transaction
        // migration state machine across those constructors.
        let _migration_guard = ENCRYPTION_MIGRATION_LOCK.lock().await;
        let state = self
            .database
            .query(
                "SELECT state FROM userstore_schema_migrations WHERE version=$1",
                &[&ENCRYPTION_MIGRATION_VERSION],
            )
            .await
            .context("reading UserStore encryption migration state")?;
        ensure!(state.len() <= 1, "duplicate UserStore migration version");
        if state
            .first()
            ...
            == Some("complete")
        {
            return self.verify_encrypted_storage().await;
        }

        // Replace #1385's plaintext-size constraints with dual-format
        // constraints atomically. A crash can leave only a fully valid
        // transitional state, never an unconstrained algorithm/PQ pairing.
        let tx = self.database.transaction().await?;
        if let Err(error) = tx.exec(BEGIN_ENCRYPTION_MIGRATION_SQL).await {
            tx.rollback()
                .await
                .context("rolling back migration setup")?;
            return Err(error).context("installing resumable encryption migration");
        }
        tx.commit().await.context("committing migration setup")?;
        ...
        loop {
            let rows = self
                ...
                .query(
                    "SELECT u.username \
                     FROM users u \
                     LEFT JOIN userstore_migration_progress p \
                       ON p.version=$1 AND p.username=u.username \
                     WHERE p.username IS NULL \
                     ORDER BY u.username LIMIT 1",
                    &[&ENCRYPTION_MIGRATION_VERSION],
                )
                .await
                .context("selecting next UserStore migration row")?;
            let Some(row) = rows.first() else {
                break;
            };
            let username: String = row.get(0).context("decoding migration username")?;
            self.migrate_one_user(&username)
                .await
                .with_context(|| format!("migrating encrypted UserStore row for {username}"))?;
        }

        let tx = self.database.transaction().await?;
        let result: Result<()> = async {
            // Freeze every table participating in the backfill before checking
            // completeness. The marker and ciphertext-only constraints are
            // then installed in this same transaction, so no writer can slip a
            // keyless/plaintext row into the verification-to-finalization gap.
            tx.exec(
                "LOCK TABLE users, pubkeys, user_encryption_keys, \
                 userstore_migration_progress IN ACCESS EXCLUSIVE MODE",
            )
            .await
            .context("locking UserStore migration tables")?;

            let missing_keys = tx
                .query(
                    "SELECT COUNT(*) FROM users u LEFT JOIN user_encryption_keys k \
                     ON k.username=u.username WHERE k.username IS NULL",
                    &[],
                )
                .await?;
            let missing_keys: i64 = missing_keys[0].get(0)?;
            ensure!(
                missing_keys == 0,
                "{missing_keys} users have no wrapped DEK"
            );

            let missing_progress = tx
                .query(
                ...
                .await?;
            let missing_progress: i64 = missing_progress[0].get(0)?;
            ensure!(
                missing_progress == 0,
                "{missing_progress} users lack durable migration progress"
            );

            tx.exec(FINALIZE_ENCRYPTION_MIGRATION_SQL)
                .await
                .context("installing final encrypted-storage constraints")
        }
        .await;
        if let Err(error) = result {
            tx.rollback()
                .await
                .context("rolling back migration finalization")?;
            return Err(error);
        }
        tx.commit()
            .await
            .context("committing encryption migration finalization")
    }

    async fn migrate_one_user(&self, username: &str) -> Result<()> {
        let tx = self.database.transaction().await?;
        let result: Result<()> = async {
            let already_done = tx
                .query(
                    "SELECT 1 FROM userstore_migration_progress \
                     WHERE version=$1 AND username=$2",
                    &[&ENCRYPTION_MIGRATION_VERSION, &username],
                )
                .await?;
            ...
            let profiles = tx
                .query(
                    "SELECT name, email, external_id FROM users \
                     WHERE username=$1 FOR UPDATE",
                    &[&username],
                )
                .await?;
            ensure!(profiles.len() == 1, "migration user disappeared");
            ...
            let wrapped = tx
                ...
                .await?;
            ensure!(wrapped.len() <= 1, "duplicate wrapped UserStore DEK");
            let had_key = !wrapped.is_empty();
            let root = if let Some(row) = wrapped.first() {
                let wrapped: Vec<u8> = row.get(0).context("decoding wrapped UserStore DEK")?;
                self.cipher.open_user_key(&wrapped).await?
            } else {
                self.create_user_key(&tx, username).await?
            };

            let profile = &profiles[0];
            let name = self.migrate_text_value(
                &root,
                username,
                EncryptedColumn::ProfileName,
                profile.get(0).context("decoding legacy name")?,
                had_key,
            )?;
            let email = self.migrate_text_value(
                &root,
                username,
                EncryptedColumn::ProfileEmail,
                profile.get(1).context("decoding legacy email")?,
                had_key,
            )?;
            let external_id = self.migrate_text_value(
                &root,
                username,
                EncryptedColumn::ProfileExternalId,
                profile.get(2).context("decoding legacy external ID")?,
                had_key,
            )?;
            tx.query(
                "UPDATE users SET name=$2, email=$3, external_id=$4 WHERE username=$1",
                &[&username, &name, &email, &external_id],
            )
            .await?;

            let keys = tx
                .query(
                    "SELECT fingerprint, pubkey, label, algorithm, pq_pubkey \
                     FROM pubkeys WHERE username=$1 ORDER BY fingerprint FOR UPDATE",
                    &[&username],
                )
                .await?;
            for row in keys {
                let fingerprint: String = row.get(0).context("decoding migration fingerprint")?;
                let algorithm_raw: String = row.get(3).context("decoding migration algorithm")?;
                let algorithm = KeyAlgorithm::parse(&algorithm_raw)?;
                let pubkey = self.migrate_public_key(
                    &root,
                    username,
                    &fingerprint,
                    row.get(1).context("decoding legacy public key")?,
                    had_key,
                )?;
                let label = self.migrate_text_value(
                    &root,
                    username,
                    EncryptedColumn::PublicKeyLabel {
                        fingerprint: &fingerprint,
                    },
                    row.get(2).context("decoding legacy key label")?,
                    had_key,
                )?;
                let pq_pubkey = self.migrate_pq_key(
                    &root,
                    username,
                    &fingerprint,
                    algorithm,
                    row.get(4).context("decoding legacy PQ key")?,
                    had_key,
                )?;
                tx.query(
                    "UPDATE pubkeys SET pubkey=$2, label=$3, pq_pubkey=$4 \
                     WHERE fingerprint=$1 AND username=$5",
                    &[&fingerprint, &pubkey, &label, &pq_pubkey, &username],
                )
                .await?;
            }

            tx.query(
                "INSERT INTO userstore_migration_progress(version, username) \
                 VALUES($1, $2)",
                &[&ENCRYPTION_MIGRATION_VERSION, &username],
            )
            .await?;
        ...
        match result {
            Ok(()) => tx
                .commit()
                .await
                .context("committing migrated UserStore row"),
            Err(error) => {
                tx.rollback()
                    .await
                    .context("rolling back migrated UserStore row")?;
                Err(error)
            }
        }
    }

    fn migrate_text_value(
        &self,
        ...
        if ColumnCipher::is_ciphertext(&value) {
            ensure!(
                had_key,
                "encrypted UserStore text has no wrapped DEK; refusing to resurrect shredded data"
            );
            let plaintext = self.cipher.open_raw(root, username, column, &value)?;
            std::str::from_utf8(&plaintext).context("encrypted UserStore text is not UTF-8")?;
            return Ok(Some(std::mem::take(&mut *value)));
        }
        std::str::from_utf8(&value).context("legacy UserStore text is not UTF-8")?;
        Ok(Some(self.cipher.seal_raw(root, username, column, &value)?))
    }

    fn migrate_public_key(
        &self,
        root: &Zeroizing<[u8; ROOT_DEK_BYTES]>,
        username: &str,
        fingerprint: &str,
        ...
        if ColumnCipher::is_ciphertext(&value) {
                ...
                had_key,
                "encrypted public key {fingerprint} has no wrapped DEK; refusing to resurrect shredded data"
            );
            let plaintext = self.cipher.open_raw(
                root,
                username,
                EncryptedColumn::PublicKey { fingerprint },
                &value,
            )?;
            Self::validate_migration_public_key(fingerprint, &plaintext)?;
            return Ok(std::mem::take(&mut *value));
        }
        Self::validate_migration_public_key(fingerprint, &value)?;
        self.cipher.seal_raw(
            root,
            username,
            EncryptedColumn::PublicKey { fingerprint },
            &value,
    ...
    fn validate_migration_public_key(fingerprint: &str, value: &[u8]) -> Result<()> {
        let bytes: [u8; 32] = value
            .try_into()
            .map_err(|_| anyhow!("legacy Ed25519 key {fingerprint} is not 32 bytes"))?;
        let key = VerifyingKey::from_bytes(&bytes)
            .with_context(|| format!("decoding legacy Ed25519 key {fingerprint}"))?;
        ensure!(
            super::pubkey_fingerprint(&key) == fingerprint,
            "legacy key does not match fingerprint {fingerprint}"
        );
        Ok(())
    }
    ...
    fn migrate_pq_key(
        ...
        match (algorithm.is_hybrid(), value) {
            (false, None) => Ok(None),
            (true, Some(value)) => {
                let mut value = Zeroizing::new(value);
                if ColumnCipher::is_ciphertext(&value) {
                        ...
                        had_key,
                        "encrypted PQ key {fingerprint} has no wrapped DEK; refusing to resurrect shredded data"
                    );
                    let plaintext = self.cipher.open_raw(
                        root,
                        username,
                        EncryptedColumn::PqPublicKey { fingerprint },
                        &value,
                    )?;
                    ensure!(
                        plaintext.len() == 1952,
            ...
            (true, None) => bail!("hybrid key {fingerprint} has no ML-DSA-65 material"),
            (false, Some(_)) => bail!("classical key {fingerprint} carries ML-DSA-65 material"),
        }
    }

    async fn verify_encrypted_storage(&self) -> Result<()> {
        ...
        let plaintext_profiles = self
            .database
            .query(
                "SELECT COUNT(*) FROM users WHERE \
                 (name IS NOT NULL AND (octet_length(name) < 32 \
                    OR substring(name from 1 for 4) <> decode('48534331','hex'))) \
                 OR (email IS NOT NULL AND (octet_length(email) < 32 \
                    OR substring(email from 1 for 4) <> decode('48534331','hex'))) \
                 OR (external_id IS NOT NULL AND (octet_length(external_id) < 32 \
                    OR substring(external_id from 1 for 4) <> decode('48534331','hex')))",
                &[],
            )
            .await?;
        let plaintext_profiles: i64 = plaintext_profiles[0].get(0)?;
        ensure!(
            plaintext_profiles == 0,
            "{plaintext_profiles} users retain plaintext profile values"
        );

        let invalid_keys = self
            .database
            .query(
                "SELECT COUNT(*) FROM pubkeys WHERE \
                 octet_length(pubkey) <> 64 \
                 OR substring(pubkey from 1 for 4) <> decode('48534331','hex') \
                 OR (label IS NOT NULL AND (octet_length(label) < 32 \
                     OR substring(label from 1 for 4) <> decode('48534331','hex'))) \
                 OR NOT ( \
                   (algorithm='ed25519' AND pq_pubkey IS NULL) \
                   OR (algorithm='ed25519+ml-dsa-65' \
                       AND pq_pubkey IS NOT NULL \
                       AND octet_length(pq_pubkey)=1984 \
                       AND substring(pq_pubkey from 1 for 4)=decode('48534331','hex')) \
                 )",
            ...
            .await?;
        let invalid_keys: i64 = invalid_keys[0].get(0)?;
        ensure!(
            invalid_keys == 0,
            "{invalid_keys} public keys violate encrypted storage invariants"
        );
        Ok(())
    }
...
mod tests {
    ...
    async fn prepare_legacy_schema(store: &PgliteUserStore) {
        store
            .database
        ...
        store
            .database
            .query(
                "DELETE FROM userstore_schema_migrations WHERE version=$1",
                &[&ENCRYPTION_MIGRATION_VERSION],
            )
            .await
            .unwrap();
        let tx = store.database.transaction().await.unwrap();
        tx.exec(BEGIN_ENCRYPTION_MIGRATION_SQL).await.unwrap();
    ...
    #[tokio::test]
    async fn migration_encrypts_legacy_profile_and_pubkey_rows() {
        let guard = TEST_LOCK.lock().await;
        let store = shared().await;
        prepare_legacy_schema(store).await;

        let key = make_key();
        let fingerprint = super::super::pubkey_fingerprint(&key);
        let name = b"Legacy Alice".to_vec();
        let email = b"legacy.alice@example.test".to_vec();
        let public_key = key.to_bytes().to_vec();
        let label = b"legacy laptop".to_vec();
        store
            .database
            .query(
                "INSERT INTO users(username, sub, name, email) VALUES($1,$2,$3,$4)",
                &[&"alice", &"legacy-sub", &name, &email],
            )
            .await
            .unwrap();
        store
            .database
            .query(
                "INSERT INTO pubkeys(fingerprint,username,pubkey,label,algorithm,created_at) \
                 VALUES($1,$2,$3,$4,'ed25519',1)",
                &[&fingerprint, &"alice", &public_key, &label],
            )
            .await
            .unwrap();

        store.migrate_encrypted_columns().await.unwrap();

        let rows = store
            .database
            .query(
                "SELECT u.name,u.email,p.pubkey,p.label \
                 FROM users u JOIN pubkeys p ON p.username=u.username \
            ...
            .unwrap();
        for index in 0..4 {
            let raw: Vec<u8> = rows[0].get(index).unwrap();
            assert!(ColumnCipher::is_ciphertext(&raw));
        }
        let profile = store.get_profile("alice").await.unwrap().unwrap();
        assert_eq!(profile.name.as_deref(), Some("Legacy Alice"));
        assert_eq!(profile.email.as_deref(), Some("legacy.alice@example.test"));
        let keys = store.list_pubkeys("alice").await.unwrap();
        assert_eq!(keys.len(), 1);
        assert_eq!(keys[0].label.as_deref(), Some("legacy laptop"));
        ...
        let state = store
            ...
            .await
            .unwrap();
        assert_eq!(state[0].get::<String>(0).unwrap(), "complete");

        // The #1385 algorithm/PQ invariant survives constraint replacement.
        assert!(store
            .database
            .query(
    ...
    #[tokio::test]
    async fn migration_resumes_without_reencrypting_completed_users() {
        let guard = TEST_LOCK.lock().await;
        let store = shared().await;
        prepare_legacy_schema(store).await;
        for (username, name) in [("alice", "Legacy Alice"), ("bob", "Legacy Bob")] {
            let sub = format!("{username}-sub");
            let bytes = name.as_bytes().to_vec();
            store
                .database
                .query(
                    "INSERT INTO users(username,sub,name) VALUES($1,$2,$3)",
                    &[&username, &sub, &bytes],
                )
                .await
                .unwrap();
        ...
        store.migrate_one_user("alice").await.unwrap();
        let before = store
            .database
            .query("SELECT name FROM users WHERE username='alice'", &[])
            .await
            .unwrap()[0]
            .get::<Vec<u8>>(0)
            .unwrap();
        ...
        let after = store
            ...
            .get::<Vec<u8>>(0)
            .unwrap();
        assert_eq!(before, after, "completed user was encrypted twice");
        assert_eq!(
            store
                .get_profile("bob")
                .await
    ...
    #[tokio::test]
    async fn migration_rolls_back_key_and_row_on_invalid_legacy_plaintext() {
        let guard = TEST_LOCK.lock().await;
        let store = shared().await;
        prepare_legacy_schema(store).await;
        ...
        store
            ...
            .query(
                "INSERT INTO users(username,sub,name) VALUES($1,$2,$3)",
                &[&"alice", &"legacy-sub", &invalid_name],
            )
            .await
            .unwrap();

        assert!(store.migrate_encrypted_columns().await.is_err());
        let keys = store
            .database
            .query(
                "SELECT COUNT(*) FROM user_encryption_keys WHERE username='alice'",
                &[],
            )
            .await
            .unwrap();
        assert_eq!(keys[0].get::<i64>(0).unwrap(), 0);
        let progress = store
            .database
            .query(
                "SELECT COUNT(*) FROM userstore_migration_progress \
                 WHERE version=$1 AND username='alice'",
                &[&ENCRYPTION_MIGRATION_VERSION],
            )
            .await
            .unwrap();
        assert_eq!(progress[0].get::<i64>(0).unwrap(), 0);
        let stored = store
            .database
            .query("SELECT name FROM users WHERE username='alice'", &[])
            .await
            ...
            .get::<Vec<u8>>(0)
            .unwrap();
        assert_eq!(stored, invalid_name);

        let corrected = b"Legacy Alice".to_vec();
        store
            .database
            .query(
                "UPDATE users SET name=$1 WHERE username='alice'",
                &[&corrected],
            )
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn migration_does_not_resurrect_ciphertext_with_a_deleted_dek() {
        let (guard, store) = fresh().await;
        store.register("alice").await.unwrap();
        store
            .set_profile(
                "alice",
        ...
        store
            .database
            .query("DELETE FROM users WHERE username='alice'", &[])
            .await
            .unwrap();
        store.migrate_encrypted_columns().await.unwrap();
        drop(guard);
