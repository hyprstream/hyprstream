...
impl TokenSampler {
    ...
    pub fn sample(&mut self, logits: &Tensor) -> Result<u32> {
        ...
        if let Ok(logits_vec) = Vec::<f32>::try_from(logits.shallow_clone()) {
            ...
            let min = logits_vec
                ...
                .ok_or_else(|| anyhow!("Empty logits tensor"))?;
            warn!(
                "DEBUG: Logits shape: {:?}, min={:.3}, max={:.3}",
                logits.size(),
                min,
                max
            );
    ...
    fn apply_repetition_penalty(&self, logits: &Tensor) -> Result<Tensor> {
        let mut logits_vec: Vec<f32> = Vec::try_from(logits.shallow_clone())
        ...
        Ok(Tensor::from_slice(&logits_vec)
    ...
    fn top_k_top_p_sampling(&mut self, logits: &Tensor) -> Result<u32> {
        ...
        let mut probs_vec: Vec<f32> = Vec::try_from(probs)
            .map_err(|e| anyhow::anyhow!("Tensor conversion failed: {:?}", e))?;
    ...
    fn typical_p_sampling(&mut self, logits: &Tensor, typical_p: f32) -> Result<u32> {
        let probs = logits.softmax(0, tch::Kind::Float);
        let probs_vec: Vec<f32> = Vec::try_from(probs)
            .map_err(|e| anyhow::anyhow!("Tensor conversion failed: {:?}", e))?;
        ...
        let mut diffs: Vec<(usize, f32, f32)> = probs_vec
            .iter()
            .enumerate()
            .filter(|(_, &p)| p > 0.0)
            .map(|(i, &p)| {
