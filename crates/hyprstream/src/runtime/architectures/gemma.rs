...
use crate::runtime::tensor_helpers::{
    broadcast_add, broadcast_mul, broadcast_sub, dims3, dims4, scalar_tensor, square_tensor,
};
...
impl GemmaModel {
    ...
    pub fn from_safetensors(path: &Path, device: &Device, dtype: DType) -> Result<Self> {
        ...
        let config = if config_path.exists() {
            ...
            tracing::info!(
                "Loaded Gemma3 config: hidden_size={}, num_layers={}, vocab_size={}",
                parsed_config.hidden_size,
                parsed_config.num_hidden_layers,
                parsed_config.vocab_size
            );
            tracing::info!(
                "  activation={}, query_pre_attn_scalar={:?}",
                parsed_config.hidden_activation,
                parsed_config.query_pre_attn_scalar
            );
            parsed_config
        } else {
        ...
        let parent_dir = path.parent().ok_or_else(|| anyhow!("Invalid model path"))?;
    ...
    fn parse_config(config_str: &str) -> Result<GemmaConfig> {
        ...
        let query_pre_attn_scalar = config.get("query_pre_attn_scalar").and_then(|v| {
...
impl ModelOperations for GemmaModel {
    ...
    fn forward(&self, input: &Tensor, _past_kv: Option<&Tensor>) -> Result<Tensor> {
        ...
        let mut hidden_states = if let Some(embed) = &self.embed_tokens {
            ...
            tracing::debug!(
                "Applied Gemma embedding scaling by sqrt(hidden_size={}) = {}",
                hidden_size,
                scale
            );
...
impl GemmaModel {
    ...
    pub fn from_varstore(vs: &tch::nn::VarStore, device: &Device, dtype: DType) -> Result<Self> {
        ...
        if let Some(embed) = weights.get("model.embed_tokens.weight") {
            ...
            info!(
                "📝 Found embeddings: {} -> [{}, {}]",
                "model.embed_tokens.weight", shape[0], shape[1]
            );
        }
    ...
    pub fn load_weights_from_varstore(&mut self, vs: &tch::nn::VarStore) -> Result<()> {
        ...
        info!(
            "🔍 Validating {} loaded tensors for Gemma architecture",
            variables.len()
        );
