    ...
    pub fn to_dtype(&self) -> Result<DType> {
        match self {
            FP8Format::E4M3 => {
                ...
                Err(anyhow!(
                    "F8E4M3 not yet available as DType in candle-core 0.9.1. Use BF16 instead."
                ))
            }
            FP8Format::E5M2 => {
                // E5M2 not yet supported in candle
                Err(anyhow!(
                    "F8E5M2 not yet supported in candle-core. Use BF16 instead."
                ))
            }
        }
    }
...
impl FP8Engine {
    ...
    pub fn quantize_model(&mut self, weights: Vec<Tensor>) -> Result<()> {
        ...
        for weight in weights {
            ...
            // Quantize to FP8
            let (fp8_weight, _scale) = self
                .config
                .scaler
                .quantize(&weight, self.config.forward_format)?;
    ...
    pub fn backward(&mut self, grad_output: &Tensor) -> Result<Tensor> {
        ...
        let (grad_fp8, scale) = if self.config.backward_format.is_supported() {
            self.config
                .scaler
                .quantize(grad_output, self.config.backward_format)?
        } else {
            // Fallback to BF16
        ...
        let grad_input =
            self.config
                .scaler
                .dequantize(&grad_fp8, scale, self.config.master_dtype)?;
