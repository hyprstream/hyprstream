...
"#;
    ...
    fn parse_policy_schema() {
        let schema = parse_capnp_schema(POLICY_SCHEMA, "policy").expect("parse schema");
    ...
    fn parse_struct_fields() {
        let schema = parse_capnp_schema(POLICY_SCHEMA, "policy").expect("parse schema");
        let check_struct = schema.structs.iter().find(|s| s.name == "PolicyCheck").expect("find struct PolicyCheck");
        ...
        let issue_struct = schema.structs.iter().find(|s| s.name == "IssueToken").expect("find struct IssueToken");
    ...
    fn parse_schema_with_enums() {
        let schema = parse_capnp_schema(ENUM_SCHEMA, "model").expect("parse schema");
        assert_eq!(schema.enums.len(), 1);
        assert_eq!(schema.enums[0].name, "Status");
        assert_eq!(schema.request_variants.len(), 2);
        assert_eq!(schema.response_variants.len(), 2);

        let info_struct = schema.structs.iter().find(|s| s.name == "ModelInfo").expect("find struct ModelInfo");
    ...
    fn parse_scoped_clients() {
        let schema = parse_capnp_schema(SCOPED_SCHEMA, "registry").expect("parse schema");
    ...
    fn parse_variant_line_basic() {
        ...
        let v = parse_variant_line("check @1 :PolicyCheck;").expect("parse variant");
    ...
    fn parse_field_line_basic() {
        let f = parse_field_line("subject @0 :Text;").expect("parse field");
        assert_eq!(f.name, "subject");
        assert_eq!(f.type_name, "Text");

        let f = parse_field_line("ttl @1 :UInt32;").expect("parse field");
    ...
    fn parse_field_with_comment() {
        let f = parse_field_line("token @0 :Text; # Signed JWT").expect("parse field");
    ...
    fn collect_list_struct_types_finds_referenced() {
        ...
        assert!(types.contains(&"ModelInfo".to_owned()));
    }
...
"#;
    ...
    fn parse_nested_scoped_clients() {
        let schema = parse_capnp_schema(NESTED_SCOPED_SCHEMA, "registry").expect("parse schema");
    ...
    fn has_union_flag_set() {
        let structs = parse_all_structs(POLICY_SCHEMA);
        let req = structs.iter().find(|s| s.name == "PolicyRequest").expect("find struct PolicyRequest");
        assert!(req.has_union);
        let check = structs.iter().find(|s| s.name == "PolicyCheck").expect("find struct PolicyCheck");
...
"#;
        ...
        let foo = structs.iter().find(|s| s.name == "Foo").expect("find struct Foo");
        assert_eq!(foo.fields.len(), 1);
        assert_eq!(foo.fields[0].name, "value");
        assert_eq!(foo.fields[0].type_name, "UInt32");
        assert!(!foo.has_union);

        let bar = structs.iter().find(|s| s.name == "Bar").expect("find struct Bar");
        ...
        let multi = structs.iter().find(|s| s.name == "MultiLine").expect("find struct MultiLine");
