...
struct McpRequest {
  ...
  union {
    # Get service status
    getStatus @1 :Void $scope(query) $dispatchMac("internal:pq-hybrid") $mcpDescription("Get MCP service status");

    # List available tools
    listTools @2 :Void $scope(query) $dispatchMac("internal:pq-hybrid") $mcpDescription("List available MCP tools");
