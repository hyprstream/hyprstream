...
- **F5 (amount/phase invariants at the seam) — partially fixed, remainder accepted with owner.** `TransferId` is now a newtype whose derivation from `operation_id` is architecture-owned (§4). `DualAttestation::join` enforces `amount <= max_charge` and unit equality at finalization. A `BoundedCharge` newtype over `(reservation, actual)` was judged redundant with the join check at this seam; the `EconomicAuthority::post`/`compensate` phase preconditions remain documented MUSTs on the trait. Enforcement inside the ledger implementation: #1072.
...
- **F7 (tautological tests / unused dep / unpinned time) — fixed in the scaffold.** Negative-control tests now cover digest/bytes mismatch, crossed-attestation rejection, unposted-ledger rejection, and forged-finalization rejection; `thiserror` was dropped; `expires_at` is pinned to Unix seconds on the registrar's monotonic-clamped time basis. #1065/#1069 inherit the requirement that validating constructors ship with mutation-effective negative tests.
