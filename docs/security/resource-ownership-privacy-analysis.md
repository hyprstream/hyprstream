...
Operational audit, trust, and observability logs retain event-level opaque operation IDs for **30 days** and then retain only daily aggregated, non-identifier metrics for **13 months**. Access is limited to the on-call security/registrar operations role and incident responders approved for the affected service; routine product, analytics, support, and issuer users have no raw-log access. Exports are prohibited except a case-scoped, access-controlled incident package approved by security and recorded in the audit trail; such packages expire after 30 days. Aggregation removes operation IDs and replaces timestamps with daily buckets. Deletion is automated and audited; legal hold is case-scoped, approved by counsel/security, and does not authorize general analytics reuse. These rules apply equally to opaque IDs because time-indexed opaque IDs are still a correlation database.
