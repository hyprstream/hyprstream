...
1. **Cross-claim substitution:** resource, operation, origin, unit, payer, policy, predecessor, carrier, or profile is changed between attesters. Control: both independently derive exact canonical bytes; attestations embed the digest only and join through a validating constructor that rejects crossed digests, wrong key purpose, unposted phase, and unit/amount mismatch; the registrar compares the digest against the canonical bytes it holds.
2. **Payment-as-title / authorization-as-credit:** one valid attestation is reused as both. Control: distinct types, signer authorities, domain separators, and mandatory dual verification.
...
5. **Premature visibility:** staged or posted bytes leak via CAS/9P. Control: non-public staging identity; namespace consumes only `Finalized`, whose value is constructible solely from a registrar-signed finalization statement verified against accepted-current registrar authority, so a forged finalization cannot publish; the public projection is privacy-minimized; quarantine namespace is separately authorized.
