...
3. **Detect partial finalization.** A transition recorded as `spend-committed` but not `finalized` is re-evaluated: if both attestations still hold over the identical canonical digest, finalize once; otherwise void or quarantine.
