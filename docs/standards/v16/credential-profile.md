...
Status: gate-2 input draft. This document freezes the credential side of the
v16 dispatch profile: the claims table, the credential/session identifier
rules, the one-shot versus reusable profiles, and the revocation semantics that
the request-proof profile cross-references. It is not operator-approved and
...
| Audience | `aud` | `aud` (3) | REQUIRED | Exact service/resource audience (RFC 8707 resource indicator semantics). Wrong audience denies. |
...
| Tenant | `tenant` | `tenant` | REQUIRED | Verified tenant/Casbin domain. Missing, empty, or wildcard tenants deny. |
| Clearance | `clearance` | `clearance` | REQUIRED for authenticated dispatch | Authority-issued MAC clearance (`Level`/`Compartments`). Assurance is **not** carried here: it is derived from verified key material. A claim may restrict assurance but can never raise it. A clearance-less credential denies. |
...
| User session | `sid` | — | REQUIRED for OIDC/user session credentials; otherwise absent | See §3. |
| Workload session | `workload_session_id` | `workload_session_id` | Only for an issuer-managed workload credential family | See §3. |
...
The claims table above is a restatement of the controlling design; the
following are proposals for gate-2 approval:

1. `tenant`, `clearance`, and `workload_session_id` are carried under those
   exact names in both encodings, with the CWT encoding using **text** claim
   keys rather than private-use integer keys, so that no integer allocation is
   frozen before a CWT session profile exists.
2. `clearance` encodes the `Level` and `Compartments` axes only; assurance is
   structurally absent from the credential rather than present-and-ignored.
