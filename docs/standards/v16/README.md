...
| [`vectors/`](vectors) | Machine-readable vectors: 10 positive, 66 negative, the seeded test keys, the replay-namespace thumbprint vectors (`proof-v1-thumbprints.json`), and the authenticated credential context (`proof-v1-credentials.json`: the frozen `verifier_now` clock, the issuer-signed at+jwt tokens the authenticated positives hash, the configured trusted issuer, and the authoritative `(iss, jti)` credential-revocation store) |
