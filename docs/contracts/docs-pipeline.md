...
Run `python3 tools/check-docs-catalog.py`. The checker asks Git for tracked `.capnp` files, so a new schema and a stale removed schema both fail. It validates every schema license against its owner Cargo manifest, distinguishes persisted-CGR producers from capnp-only test compilation, and verifies each declared build/import root. It compares the catalog to actual CLI builder, MCP registrations, factory attributes and feature gates, VFS dispatch registrations, hidden/streaming policies, and tracked TS/JS sources. It rejects duplicate source IDs or service identities, omitted exclusion reasons, widened public-prose patterns, non-SPDX publication licenses, package-policy drift, and a catalog that stops naming the ontology as authority.
