...
Run `python3 tools/check-docs-catalog.py`. The checker asks Git for tracked `.capnp` files, so a new schema and a stale removed schema both fail. It derives every schema owner from an audited owner directory and that directory's Cargo manifest, distinguishes persisted-CGR producers from capnp-only test compilation, and verifies exact source/import arguments for every declared build root. It compares the catalog to actual CLI builder (including the deliberately restricted manual `discovery ping` registration), MCP registrations, factory attributes and feature gates, VFS dispatch registrations, schema-derived hidden/streaming method inventories, and tracked TS/JS sources. It rejects duplicate source IDs or service identities, omitted exclusion reasons, widened public-prose patterns, non-SPDX publication licenses, package-policy drift, a stale provenance pair, and a catalog that stops naming the ontology as authority.
