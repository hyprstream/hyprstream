...
**Status:** settled (#578 merged). Revision history: `did:at9p` resolution semantics amended (#1097, §4.1/§5/§6); `did:at9p` atproto-OAuth-flow terms amended (#1097 rev 1, §10); §10 precision-revised (#1097 rev 2 — this revision: present-tense AS claims → tracked language + #1112 dep, server-side `did:plc` resolution specified, `getServiceAuth` token authority + replay properties + known DPoP-binding gap, §10.4 fail-closed for native `did:at9p` assertions). Supersedes prior §10 text by *correction*; §§1–9 unchanged.
**Canonical owner:** `hyprstream` — `crates/hyprstream-rpc/src/identity.rs` (`Did`).
**Source issues/PRs:** #563 (design + sign-off) · #578 (def + field-level `$domainType`) · #579 (resolver + assurance, PQC pathway) · #1097 (`did:at9p` amendment — resolution + OAuth-flow terms, rev 2) · #1112 (authenticated `com.atproto.*` XRPC surface — **open** dependency of the flow) · #1113 (atproto OAuth AS conformance profile — **open**; `sub` scope excludes `did:at9p`) · #1114 (HTTPS `did:at9p` verification — **open**) · #948 (OAuth conformance spike — **open**).
...
| Server-side `did:plc` issuer resolution (for the DID→UCAN exchange) | **hyprstream** — required new `MethodDispatchResolver` arm (PLC base configurable, default `https://plc.directory`, §9); tracked under #1113. Until it lands, `did:plc` service-auth issuers fail closed (§10.2.1) |
| DID→UCAN exchange endpoint (mint + verify) | **hyprstream** (`services/oauth`, §10.2) — endpoint; **www-cyberdione-ai** `atproto-identity-exchange.md` owns the spec. No token→DPoP-key binding on the `getServiceAuth` surface — known gap (§10.2.2) |
...
`did:at9p` is **not** a registered atproto DID method and **never** appears in the atproto OAuth login flow (no PAR, no DPoP, no `@atproto/oauth-client-browser` session). A `did:at9p` login is handled by a **DID→UCAN exchange on the hyprstream side**: the client presents an atproto service-auth token (`com.atproto.server.getServiceAuth`) whose `aud` is the hyprstream host's DID and whose `exp` is short (minutes); the host resolves the caller's atproto DID **itself** (per §4, never trusting a client-supplied document/key), verifies the token signature against the resolved classical verification key, and on success **mints a hyprstream UCAN** for a subject derived from the atproto DID. Note: `getServiceAuth` accepts `aud` / `exp` / optional `lxm` (and mints a JWT carrying `iss` / `aud` / `exp` / optional `lxm` / `jti`) — it has **no** generic `scope` field and **no** `cnf`/DPoP-key member; the exact authority + replay posture this yields is specified in §10.2.2, not assumed.
...
- This arm is tracked as a prerequisite of the exchange endpoint under **#1113** (the conformance epic owns server-side resolution of the atproto login population); a dedicated resolver follow-up may be split off.

This reconciles the §2/§8/§9 statement that "hyprstream does not resolve `did:plc`" (true for general/frontend use) with §10.2's host-side requirement: the exchange endpoint is the one place a server-side `did:plc` arm is required, and it is scoped to this exchange, not a general obligation.
...
- **Fields actually available:** `aud` (must equal the host's DID — reject otherwise), `exp` (short, minutes), optional `lxm` (a lexicon method NSID the token authorizes), and `jti` (a unique token id). There is **no `cnf` member** and **no DPoP-key binding** in the JWT.
- **Concrete `lxm` pin:** the host MUST require `lxm` = the exchange endpoint's lexicon NSID (so the token is scoped to this one call, not reusable against any other method the host exposes). That NSID is fixed when the endpoint is registered (tracked under #1113's implementation); this contract pins the **requirement** (`lxm` MUST be present and MUST equal the exchange method), not a synthesized NSID.
- **Replay properties that actually hold:** short `exp` (minutes), audience-pin (`aud` = host DID ⇒ only the named host accepts it), `lxm` pin (limits the token to the exchange method), and **`jti` enforced as one-time-use server-side** (the host records each accepted `jti` for the duration of `exp` and rejects replays).
- **Known gap (do not paper over it):** a stolen service-auth JWT is replayable by a third party within its short lifetime — the token carries **no proof that the presenter holds the original atproto session key**. A real token→DPoP-key cryptographic linkage does not exist on the atproto `getServiceAuth` surface today; this contract MUST NOT claim one. Hardening (e.g. binding the exchange to the atproto session DPoP key via an out-of-band proof the host can verify against the resolved PDS) is a **deferred security task**, tracked alongside #1113, not a property of this revision.
- **Failure modes fail closed** (§6): unresolved DID document, no verifying key, signature mismatch, `aud` ≠ host DID, expired, wrong/missing `lxm`, or a replayed `jti` never silently elevate.
