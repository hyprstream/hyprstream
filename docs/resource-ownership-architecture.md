...
The Rust mirror makes the load-bearing boundaries explicit: a canonical intent exists only after canonical decoding, predecessor-shape, resource-kind/operation content-matrix, required-assurance, and digest verification. `DualAttestation::verify_and_join` verifies each raw candidate under its own accepted-current authority, seals the CIDs, verifier-derived assurance, and resolved roots into private verified evidence, rejects crossed claims/unposted or over-limit accounting and assurance below the typed intent floor, and calls an independence policy. The baseline predicate asserts only that the MAC and ledger authorities have distinct accepted roots and signer DIDs; it does not compare `key_id` fragments because those fragments are scoped by the authority namespace and are not a globally meaningful key identity. Deployments that need to reject common key material across distinct namespaces must provide a policy using a verifier-derived globally scoped key identity or fingerprint. Service or host co-location is not cryptographic independence: a shared compromise domain must be rejected by deployment policy or recorded as an accepted residual risk, never inferred safe from role labels. `FinalizedResource` derives content-CID presence from the closed resource-kind/operation matrix, compares and retains both verified CIDs, and only it can mint publication effects.
